...
job_directory=/projects/b1059/projects/Ryan/protein_structure/High_Throughput_AF/data/03082021_ben1_tags_multimer/
files=$job_directory"fastas/"*.fa
...
for filename in $files; do 
    ...
    cpu_file="${job_directory}"scripts/"${name}_cpu.sh"
    echo "#!/bin/bash
...
alphafold-multimer --fasta_paths=$input_fasta\
    --output_dir=$job_directory"models/"$name \
...
gpu_file="${job_directory}"scripts/"${name}_gpu.sh"
echo "#!/bin/bash
#SBATCH --account=b1042  ## YOUR ACCOUNT pXXXX or bXXXX
